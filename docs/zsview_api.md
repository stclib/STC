...
zsview          zsview_from_pos(zsview zv, isize_t pos);            // subview starting from index pos

isize_t         zsview_size(zsview zv);
...
csview          zsview_subview(zsview zv, isize_t pos, isize_t len); // return as csview span
zsview          zsview_tail(zsview zv, isize_t len);                 // subview of the trailing n bytes

bool            zsview_equals(zsview zv, const char* str);
isize_t         zsview_find(zsview zv, const char* str);
...
#### UTF8 methods
```c++
zsview          zsview_u8_from_pos(zsview zv, isize_t u8pos);       // subview starting from rune u8pos
isize_t         zsview_u8_size(zsview zv);                          // number of utf8 runes
zsview_iter     zsview_u8_at(zsview zv, isize_t u8pos);             // get rune at rune position
csview          zsview_u8_subview(zsview zs, isize_t u8pos, isize_t u8len);
zsview          zsview_u8_tail(zsview zv, isize_t u8len);           // subview of the last u8len runes
...
|:---------------|:---------------------------------------------|:-------------------------|
| `zsview`       | `struct { const char *str; isize_t size; }` | The string view type   |
