...
TEST(cregex, replace)
{
    ...
    c_deferred(
        ...
        cstr_take(&str, cregex_replace_pattern_with(pattern, input, "$1/$3/$2", INT32_MAX, add_10_years, CREG_DEFAULT));
        EXPECT_STREQ(cstr_str(&str), "start date: 2025/31/12, end date: 2032/28/02");

        // Wrap first date inside []:
        cstr_take(&str, cregex_replace_pattern_with(pattern, input, "[$0]", 1, NULL, CREG_DEFAULT));
        ...
        cstr_take(&str, cregex_replace_sv_with(&re, csview_from(input), "$3.$2.$1;",
