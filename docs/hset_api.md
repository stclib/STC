...
hset_X          hset_X_with_capacity(isize_t cap);
...
bool            hset_X_reserve(hset_X* self, isize_t size);
void            hset_X_shrink_to_fit(hset_X* self);

bool            hset_X_is_empty(const hset_X* self);
isize_t         hset_X_size(const hset_X* self);                         // num. of allocated buckets
isize_t         hset_X_capacity(const hset_X* self);                     // buckets * max_load_factor
isize_t         hset_X_bucket_count(const hset_X* self);
