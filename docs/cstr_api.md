...
isize_t         cstr_size(const cstr* self);
isize_t         cstr_capacity(const cstr* self);
isize_t         cstr_to_index(const cstr* self, cstr_iter it);          // to byte position at iter.
...
zsview          cstr_tail(cstr* self, isize_t len);                     // to zsview subview of the trailing len bytes
csview          cstr_subview(const cstr* self, isize_t pos, isize_t len); // to csview subview from pos and length len

void            cstr_clear(cstr* self);
char*           cstr_reserve(cstr* self, isize_t capacity);             // return pointer to buffer
void            cstr_resize(cstr* self, isize_t len, char fill);
...
void            cstr_insert(cstr* self, isize_t pos, const char* ins);
void            cstr_insert_sv(cstr* self, isize_t pos, csview ins);

void            cstr_erase(cstr* self, isize_t pos, isize_t len);       // erase len bytes from pos
...
void            cstr_replace_at(cstr* self, isize_t pos, isize_t len, const char* repl); // replace at a pos
void            cstr_replace_at_sv(cstr* self, isize_t pos, isize_t len, const csview repl);

bool            cstr_equals(const cstr* self, const char* str);
bool            cstr_equals_sv(const cstr* self, csview sv);

isize_t         cstr_find(const cstr* self, const char* search);
isize_t         cstr_find_sv(const cstr* self, csview search);
isize_t         cstr_find_at(const cstr* self, isize_t pos, const char* search); // search from pos
...
#### UTF8 methods
```c++
cstr            cstr_u8_from(const char* str, isize_t u8pos, isize_t u8len);// make cstr from an utf8 substring
isize_t         cstr_u8_size(const cstr* self);                         // number of utf8 runes
isize_t         cstr_u8_to_index(const cstr* self, isize_t u8pos);        // get byte index at rune position
cstr_iter       cstr_u8_at(const cstr* self, isize_t u8pos);              // get rune at rune position
csview          cstr_u8_subview(const cstr* self, isize_t u8pos, isize_t u8len);
zsview          cstr_u8_tail(cstr* self, isize_t u8len);                  // subview of the trailing len runes
void            cstr_u8_insert(cstr* self, isize_t u8pos, const char* ins);
void            cstr_u8_replace(cstr* self, isize_t u8pos, isize_t u8len, const char* repl);
void            cstr_u8_erase(cstr* self, isize_t u8pos, isize_t u8len);    // erase u8len runes from u8pos
...
Note that all methods with arguments `(..., const char* str, isize_t n)`, `n` must be within the range of `str` length.
...
char*           c_strnstrn(const char* str, isize_t slen, const char* needle, isize_t nlen);
```
...
| `cstr_buf`      | `struct { char *data; isize_t size, cap; }` | String buffer type   |
