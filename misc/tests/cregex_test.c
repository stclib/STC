...
TEST(cregex, compile_match_char)
{
    ...
    csview match;
    EXPECT_EQ(cregex_find_fl(&re, inp="äsdf", &match, CREG_FULLMATCH), CREG_OK);
...
TEST(cregex, search_all)
{
    ...
    c_with (cregex re = {0}, cregex_drop(&re))
    {
        ...
        inp="ab,ab,ab";
        res = cregex_find_fl(&re, inp, &m, CREG_NEXT);
...
TEST(cregex, replace)
{
    ...
    c_defer(
        ...
        cstr_take(&str, cregex_replace_pattern_tr(pattern, csview_from(input), "$1/$3/$2",
        ...
        cstr_take(&str, cregex_replace_pattern_tr(pattern, csview_from(input), "[$0]", 1, NULL, CREG_DEFAULT));
        ...
        cstr_take(&str, cregex_replace_tr(&re, csview_from(input), "$3.$2.$1;",
